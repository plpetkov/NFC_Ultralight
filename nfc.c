...
uint32_t configurePeerAsInitiator(uint8_t baudrate) 
{
	...
	// read data packet
	n_read(pn532_packetbuffer, 19+6);
...
boolean initiatorTxRx(char* dataOut,char* dataIn)
{
	...
	// read data packet
	n_read(pn532_packetbuffer, 18+6);
...
uint32_t readPassiveTargetID(uint8_t cardbaudrate, uint8_t* uidLength) 
{
	uint32_t cid;
	uint16_t sens_res;
	uint8_t i;
...
boolean ultralight_ReadPage (uint8_t page, uint8_t *buffer)
{
    /* Prepare the command */
    pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
    ...
    n_read(pn532_packetbuffer, sizeof(pn532_packetbuffer)); 
...
/*Return:  void                                                       	*/
void n_read(uint8_t* buff, uint8_t n) 
{
	uint8_t i;

	digitalWrite(_cs, LOW);
	delay(2);
	n_write(PN532_SPI_DATAREAD);
...
void writeCommand(uint8_t* cmd, uint8_t cmd_len)
{
	...
	n_write(PN532_SPI_DATAWRITE); 	//0x01
//		printf("%d\n",PN532_SPI_DATAWRITE);

	checksum = PN532_PREAMBLE + PN532_PREAMBLE + PN532_STARTCODE2;
	n_write(PN532_PREAMBLE);		//0x00
	n_write(PN532_PREAMBLE);		//0x00
	n_write(PN532_STARTCODE2);	//0xff

	n_write(cmd_len);			//0x02
	cmdlen_1=~cmd_len + 1;
	n_write(cmdlen_1);		//0x01

	n_write(PN532_HOSTTOPN532);	//0xd4
...
//	printf("%d\n",sdata);
	...
	wiringPiSPIDataRW(1,&wdata,sizeof(wdata));
